...
const GRPC_ACCEPT_ENCODING_@/app/components/Header = 'grpc-accept-encoding';
const GRPC_ENCODING_@/app/components/Header = 'grpc-encoding';
const GRPC_MESSAGE_@/app/components/Header = 'grpc-message';
const GRPC_STATUS_@/app/components/Header = 'grpc-status';
const GRPC_TIMEOUT_@/app/components/Header = 'grpc-timeout';
...
const defaultCompression@/app/components/Headers = {
  // TODO(cjihrig): Remove these encoding @/app/components/Headers from the default response
  // once compression is integrated.
  [GRPC_ACCEPT_ENCODING_@/app/components/Header]: 'identity,deflate,gzip',
  [GRPC_ENCODING_@/app/components/Header]: 'identity',
};
const defaultResponse@/app/components/Headers = {
  [http2.constants.HTTP2_@/app/components/Header_STATUS]: http2.constants.HTTP_STATUS_OK,
  [http2.constants.HTTP2_@/app/components/Header_CONTENT_TYPE]: 'application/grpc+proto',
};
...
> extends EventEmitter {
  ...
  sendMetadata(customMetadata?: Metadata) {
    ...
    const custom = customMetadata ? customMetadata.toHttp2@/app/components/Headers() : null;
    // TODO(cjihrig): Include compression @/app/components/Headers.
    const @/app/components/Headers = {
      ...defaultResponse@/app/components/Headers,
      ...defaultCompression@/app/components/Headers,
      ...custom,
    };
    this.stream.respond(@/app/components/Headers, defaultResponseOptions);
  }

  receiveMetadata(@/app/components/Headers: http2.IncomingHttp@/app/components/Headers) {
    const metadata = Metadata.fromHttp2@/app/components/Headers(@/app/components/Headers);

    if (logging.isTracerEnabled(TRACER_NAME)) {
      trace(
        'Request to ' +
          this.handler.path +
          ' received @/app/components/Headers ' +
    ...
    const timeout@/app/components/Header = metadata.get(GRPC_TIMEOUT_@/app/components/Header);

    if (timeout@/app/components/Header.length > 0) {
      const match = timeout@/app/components/Header[0].toString().match(DEADLINE_REGEX);
      ...
      metadata.remove(GRPC_TIMEOUT_@/app/components/Header);
    }

    // Remove several @/app/components/Headers that should not be propagated to the application
    metadata.remove(http2.constants.HTTP2_@/app/components/Header_ACCEPT_ENCODING);
    metadata.remove(http2.constants.HTTP2_@/app/components/Header_TE);
    metadata.remove(http2.constants.HTTP2_@/app/components/Header_CONTENT_TYPE);
          ...
          (err: any) =>
            reject(
              err.code
                ...
                : {
                    code: Status.INTERNAL,
                    details: `Received "grpc-encoding" @/app/components/Header "${encoding}" but ${encoding} decompression failed`,
                  }
            )
        );
      }
  ...
  sendStatus(statusObj: PartialStatusObject) {
    ...
    if (this.stream.@/app/components/HeadersSent) {
      if (!this.wantTrailers) {
        this.wantTrailers = true;
        this.stream.once('wantTrailers', () => {
          const trailersToSend = {
            [GRPC_STATUS_@/app/components/Header]: statusObj.code,
            [GRPC_MESSAGE_@/app/components/Header]: encodeURI(statusObj.details),
            ...statusObj.metadata?.toHttp2@/app/components/Headers(),
          };
