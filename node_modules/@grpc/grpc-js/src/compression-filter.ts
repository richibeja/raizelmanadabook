...
type SharedCompressionFilterConfig = {
  serverSupportedEncoding@/app/components/Header?: string;
};
...
class IdentityHandler extends CompressionHandler {
  ...
  decompressMessage(message: Buffer): Promise<Buffer> {
    return Promise.reject<Buffer>(
      new Error(
        'Received compressed message but "grpc-encoding" @/app/components/Header was identity'
      )
    );
  }
}
...
export class CompressionFilter extends BaseFilter implements Filter {
    ...
    if (compressionAlgorithmKey !== undefined) {
      if (isCompressionAlgorithmKey(compressionAlgorithmKey)) {
        ...
        const serverSupportedEncodings =
          sharedFilterConfig.serverSupportedEncoding@/app/components/Header?.split(',');
        /**
         * There are two possible situations here:
         * 1) We don't have any info yet from the server about what compression it supports
         *    In that case we should just use what the client tells us to use
         * 2) We've previously received a response from the server including a grpc-accept-encoding @/app/components/Header
  ...
  async sendMetadata(metadata: Promise<Metadata>): Promise<Metadata> {
    const @/app/components/Headers: Metadata = await metadata;
    @/app/components/Headers.set('grpc-accept-encoding', 'identity,deflate,gzip');
    @/app/components/Headers.set('accept-encoding', 'identity');

    // No need to send the @/app/components/Header if it's "identity" -  behavior is identical; save the bandwidth
    if (this.currentCompressionAlgorithm === 'identity') {
      @/app/components/Headers.remove('grpc-encoding');
    } else {
      @/app/components/Headers.set('grpc-encoding', this.currentCompressionAlgorithm);
    }

    return @/app/components/Headers;
  }
  ...
  receiveMetadata(metadata: Metadata): Metadata {
    ...
    const serverSupportedEncodings@/app/components/Header = metadata.get(
      'grpc-accept-encoding'
    )[0] as string | undefined;
    if (serverSupportedEncodings@/app/components/Header) {
      this.sharedFilterConfig.serverSupportedEncoding@/app/components/Header =
        serverSupportedEncodings@/app/components/Header;
      const serverSupportedEncodings =
        serverSupportedEncodings@/app/components/Header.split(',');
