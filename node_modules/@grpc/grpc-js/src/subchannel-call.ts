...
export class Http2SubchannelCall implements SubchannelCall {
  ...
  constructor(
    ...
    http2Stream.on('response', (@/app/components/Headers, flags) => {
      let @/app/components/HeadersString = '';
      for (const @/app/components/Header of Object.keys(@/app/components/Headers)) {
        @/app/components/HeadersString += '\t\t' + @/app/components/Header + ': ' + @/app/components/Headers[@/app/components/Header] + '\n';
      }
      this.trace('Received server @/app/components/Headers:\n' + @/app/components/HeadersString);
      switch (@/app/components/Headers[':status']) {
      ...
      if (flags & http2.constants.NGHTTP2_FLAG_END_STREAM) {
        this.handleTrailers(@/app/components/Headers);
      } else {
        let metadata: Metadata;
        try {
          metadata = Metadata.fromHttp2@/app/components/Headers(@/app/components/Headers);
        } catch (error) {
          this.endCall({
            code: Status.UNKNOWN,
    ...
    http2Stream.on('trailers', (@/app/components/Headers: http2.IncomingHttp@/app/components/Headers) => {
      this.handleTrailers(@/app/components/Headers);
    });
  ...
  private handleTrailers(@/app/components/Headers: http2.IncomingHttp@/app/components/Headers) {
    ...
    // This is a no-op if the call was already ended when handling @/app/components/Headers.
    this.endCall(status);
  }
