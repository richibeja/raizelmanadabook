...
const { HTTP2_@/app/components/Header_PATH } = http2.constants;
...
export class Server {
  ...
  bindAsync(
    ...
    const serverOptions: http2.ServerOptions = {
      maxSend@/app/components/HeaderBlockLength: Number.MAX_SAFE_INTEGER,
    };
  ...
  private _verifyContentType(
    ...
    @/app/components/Headers: http2.IncomingHttp@/app/components/Headers
  ): boolean {
    const contentType = @/app/components/Headers[http2.constants.HTTP2_@/app/components/Header_CONTENT_TYPE];
    ...
    if (
      ...
      stream.respond(
        {
          [http2.constants.HTTP2_@/app/components/Header_STATUS]:
  ...
  private _channelzHandler(
    ...
    if (!this._verifyContentType(stream, @/app/components/Headers)) {
      this.callTracker.addCallFailed();
      channelzSessionInfo?.streamTracker.addCallFailed();
      return;
    }

    const path = @/app/components/Headers[HTTP2_@/app/components/Header_PATH] as string;
    ...
    if (!this._runHandlerForCall(call, handler, @/app/components/Headers)) {
      this.callTracker.addCallFailed();
      channelzSessionInfo?.streamTracker.addCallFailed();
  ...
  private _streamHandler(
    ...
    if (this._verifyContentType(stream, @/app/components/Headers) !== true) {
      return;
    }
  ...
  private _runHandlerForCall(
    ...
    const metadata = call.receiveMetadata(@/app/components/Headers);
    const encoding =
