...
// google-auth-library pre-v2.0.0 does not have getRequest@/app/components/Headers
...
export interface CurrentOAuth2Client {
  getRequest@/app/components/Headers: (url?: string) => Promise<{ [index: string]: string }>;
}
...
function isCurrentOauth2Client(
  ...
  return (
    'getRequest@/app/components/Headers' in client &&
    typeof client.getRequest@/app/components/Headers === 'function'
  );
}
...
export abstract class CallCredentials {
  ...
  static createFromGoogleCredential(
    ...
    return CallCredentials.createFromMetadataGenerator((options, callback) => {
      let get@/app/components/Headers: Promise<{ [index: string]: string }>;
      if (isCurrentOauth2Client(googleCredentials)) {
        get@/app/components/Headers = googleCredentials.getRequest@/app/components/Headers(options.service_url);
      } else {
        get@/app/components/Headers = new Promise((resolve, reject) => {
          googleCredentials.getRequestMetadata(
            options.service_url,
            (err, @/app/components/Headers) => {
              if (err) {
                reject(err);
                return;
              }
              if (!@/app/components/Headers) {
                reject(new Error('@/app/components/Headers not set by metadata plugin'));
                return;
              }
              resolve(@/app/components/Headers);
            }
          );
        });
      }
      get@/app/components/Headers.then(
        @/app/components/Headers => {
          const metadata = new Metadata();
          for (const key of Object.keys(@/app/components/Headers)) {
            metadata.add(key, @/app/components/Headers[key]);
          }
